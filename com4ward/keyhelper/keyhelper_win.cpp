...
bool GetComputerSid(const wchar_t* account_name, SID* sid, DWORD sid_size)
{
    ...
    QScopedArrayPointer<wchar_t> domain_buffer(new wchar_t[kStartDomainLength]);
    ...
    BOOL success = ::LookupAccountNameW(NULL, account_name, sid,
        &sid_dword_size, domain_buffer.data(),
...
bool KeyHelper::getRawMachineId(QString* sid_string, int* volume_id)
{
    ...
    if (GetComputerNameW(computer_name, &size)) {
        ...
        if (GetComputerSid(computer_name, sid, SECURITY_MAX_SID_SIZE)) {
            *sid_string = QString::fromStdWString(ConvertSidToString(sid));
        }
    }

    // Get the system drive volume serial number.
    *volume_id = 0;
    if (!GetMACaddress(volume_id)) {
        Q_ASSERT("GetMachineId: Failed to retrieve volume serial number");
        *volume_id = 0;
    }
